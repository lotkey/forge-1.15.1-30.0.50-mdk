...
public class RoosterEntity extends TameableEntity {
	private static final Ingredient TEMPTATION_ITEMS = Ingredient.fromItems(Items.WHEAT_SEEDS, Items.MELON_SEEDS,
			Items.PUMPKIN_SEEDS, Items.BEETROOT_SEEDS, ItemInitNew.CHERRY_SEEDS.get());
	public float wingRotation;
	public float destPos;
	public float oFlapSpeed;
	public float oFlap;
	public float wingRotDelta = 1.0F;
	private static final DataParameter<Boolean> BEGGING = EntityDataManager.createKey(RoosterEntity.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> COLLAR_COLOR = EntityDataManager.createKey(RoosterEntity.class,
			DataSerializers.VARINT);
	public static final Predicate<LivingEntity> field_213441_bD = (p_213440_0_) -> {
		EntityType<?> entitytype = p_213440_0_.getType();
		return entitytype == EntityType.SHEEP || entitytype == EntityType.RABBIT || entitytype == EntityType.FOX
				|| entitytype == EntityType.COW || entitytype == EntityType.HORSE || entitytype == EntityType.WOLF
				|| entitytype == EntityType.DONKEY || entitytype == EntityType.MOOSHROOM
				|| entitytype == EntityType.MULE || entitytype == EntityType.PANDA || entitytype == EntityType.PIG
	...
	public RoosterEntity(EntityType<? extends RoosterEntity> type, World worldIn) {
		super(type, worldIn);
		this.setTamed(false);
	}
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new RoosterEntity.AvoidEntityGoal(this, LlamaEntity.class, 24.0F, 1.5D, 1.5D));
	...
	public void setAttackTarget(@Nullable LivingEntity entitylivingbaseIn) {
		if(!(entitylivingbaseIn instanceof ThiccenEntity)) {
	...
	protected SoundEvent getAmbientSound() {
		if (this.isAngry()) {
			return SoundInit.ENTITY_ROOSTER_GROWL.get();
		} else if (this.rand.nextInt(3) == 0) {
			return this.isTamed() && this.getHealth() < 10.0F ? SoundInit.ENTITY_ROOSTER_WHINE.get()
					: SoundInit.ENTITY_ROOSTER_PANT.get();
		} else {
	...
	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return SoundInit.ENTITY_ROOSTER_HURT.get();
	}
	...
	public void livingTick() {
		...
		this.oFlap = this.wingRotation;
	      ...
	      /*
	      if (!this.world.isRemote && this.isAlive() && !this.isChild()) {
	...
	public boolean processInteract(PlayerEntity player, Hand hand) {
		...
		if (itemstack.getItem() instanceof SpawnEggItem) {
			...
			return this.isOwner(player) || item == ItemInitNew.CHERRIES.get() && !this.isAngry();
		} else {
			if (this.isTamed()) {
			...
			} else if (item == ItemInitNew.CHERRIES.get() && !this.isAngry()) {
				if (!player.abilities.isCreativeMode) {
					itemstack.shrink(1);
				}
	...
	public RoosterEntity createChild(AgeableEntity ageable) {
		RoosterEntity wolfentity = ModEntityTypes.ROOSTER_ENTITY.get().create(this.world);
	...
	public boolean canMateWith(AnimalEntity otherAnimal) {
		if (otherAnimal == this) {
			...
			return false;
		} else if (!(otherAnimal instanceof RoosterEntity)) {
			return false;
		} else {
			RoosterEntity wolfentity = (RoosterEntity) otherAnimal;
	...
	public boolean shouldAttackEntity(LivingEntity target, LivingEntity owner) {
		if (!(target instanceof CreeperEntity) && !(target instanceof GhastEntity)) {
			if (target instanceof RoosterEntity) {
				RoosterEntity wolfentity = (RoosterEntity) target;
	...
	class AvoidEntityGoal<T extends LivingEntity> extends net.minecraft.entity.ai.goal.AvoidEntityGoal<T> {
		private final RoosterEntity wolf;

		public AvoidEntityGoal(RoosterEntity wolfIn, Class<T> p_i47251_3_, float p_i47251_4_, double p_i47251_5_,
				double p_i47251_7_) {
		...
		private boolean avoidLlama(LlamaEntity p_190854_1_) {
			return p_190854_1_.getStrength() >= RoosterEntity.this.rand.nextInt(5);
		}
		...
		public void startExecuting() {
			RoosterEntity.this.setAttackTarget((LivingEntity) null);
